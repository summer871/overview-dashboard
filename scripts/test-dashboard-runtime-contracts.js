...
function runDetachedPopoutContract() {
  ...
  assert(isolation.includes('normalizeFullTableV6561(session)'), 'Pop-out does not normalize nested vertical scroll owners.');
  assert(isolation.includes('restoreInlineStylesV6561(session)'), 'Pop-out does not restore original inline styles.');
...
function runDocumentAwareSharedControls() {
  ...
  assert(features.includes("CDA_DASHBOARD_FEATURES_VERSION = 'v6.558'"), 'Feature catalog is not v6.558.');
  assert(audit.includes('function detachedButtonParity()'), 'Browser audit lacks control parity.');
  assert(audit.includes('function detachedFullTables()'), 'Browser audit lacks detached-table checks.');
}
...
function runRemakeNativeHeaderContract() {
  ...
  assert(relevantSelectorBlock && !relevantSelectorBlock.includes('.remakeCardTitle'), 'Remake bootstrap watches title mutations.');
}
...
function runTatLayoutContract() {
  ...
  assert(adapter.includes("context.component.key === 'performance'"), 'Combined performance controls are missing.');
  assert(bootstrap.includes("version:'v6.564'"), 'TAT bootstrap is not v6.564.');
  assert(bootstrap.includes('performanceBeforeProducts:'), 'TAT bootstrap does not enforce analysis-first order.');
  assert(bootstrap.includes('analysisFirstLayout:'), 'TAT bootstrap does not require analysis-first mode.');
  ...
  assert(definition.indexOf("key:'performance'") < definition.indexOf("key:'product'"), 'Performance must appear before Products.');
  assert(definition.includes('tatPromiseStripV6564'), 'Compact promise strip markup is missing.');
  assert(definition.includes('tatPerformanceChartPanelV6564'), 'Full-width distribution markup is missing.');
  assert(!definition.includes("key:'late',title:'Promise Performance'"), 'Standalone Promise remains.');
  assert(!definition.includes("key:'distribution',title:'TAT Distribution'"), 'Standalone Distribution remains.');
  ...
  assert(layout.includes("version:'v6.564'"), 'TAT layout service is not v6.564.');
  ...
  assert(layout.includes('tatPromiseStackSegmentV6564'), 'Compact promise segments are missing.');
  assert(layout.includes('tatPromiseEmptyV6564'), 'Zero-data state is missing.');
  assert(layout.includes('max-height:360px!important'), 'Products height is not reduced.');
  assert(!layout.includes('tatPromisePanelV6563'), 'Rejected side panel remains.');
  ...
  assert(audit.includes('performanceBeforeProducts:performanceBeforeProducts'), 'TAT audit does not verify order.');
  assert(audit.includes('compactPromiseStrip:'), 'TAT audit does not verify compact strip.');
  assert(audit.includes('fullWidthDistribution:'), 'TAT audit does not verify full-width chart.');
...
console.log('Analysis-first TAT Performance component: passed');
console.log('Compact promise strip and full-width distribution: passed');
console.log('Performance-before-Products order: passed');
