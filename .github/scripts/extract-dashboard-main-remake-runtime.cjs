...
const defs = [
  ...
  { name: 'RemakePopoutControllerRuntimeV6339', start: '  window.popoutRemakeComponentV6230 = function popoutRemakeComponentV6339(component) {', end: '  /* v6.388 first-open optimization: use the nightly browser-ready consolidated gzip cache before falling back to monthly Drive shards. */', required: ['window.popoutRemakeComponentV6230', 'getRemakeFactorData'] },
...
for (let index = 1; index < blocks.length; index += 1) {
  if (blocks[index - 1].end !== blocks[index].start) fail('Remake semantic blocks are not contiguous: ' + blocks[index - 1].def.name + ' -> ' + blocks[index].def.name + '.');
}
if (blocks[blocks.length - 1].end !== startupPos) fail('Last Remake semantic block does not end at startup marker.');
blocks.forEach(function(block) { if (fs.existsSync(block.def.name + '.html')) fail('Destination module already exists: ' + block.def.name + '.html'); });
...
if (!next.includes(iifeMarker) || !next.includes(useStrictMarker) || !next.includes(startupMarker)) fail('DashboardMain no longer preserves the Remake IIFE shell/startup call.');

let reconstructed = next;
blocks.forEach(function(block) { reconstructed = reconstructed.replace("<?!= includeDashboardFile('" + block.def.name + "') ?>", block.content); });
...
try { new vm.Script(scriptMatch[1], { filename: sourcePath }); }
catch (error) { fail('DashboardMain composition source is not parseable after extraction: ' + error.message); }
...
const report = {
  datePt: '2026-08-21', checkpoint: 'DashboardMain Remake semantic runtime extraction', behaviorChangeIntended: false,
  source: sourcePath, sourceBytesBefore: sourceBytes, sourceSha256Before: sourceSha,
  sourceBytesAfter: byteLength(next), sourceSha256After: sha256(next), bytesRemovedFromDashboardMain: sourceBytes - byteLength(next),
  archivePath: archivePath, archiveByteForByteVerified: true, compositionByteForByteReconstructionVerified: true,
  iifeShellPreserved: true, startupCallPreserved: true, maxModuleBytes: maxModuleBytes,
  modules: blocks.map(function(block) { return { name: block.def.name, path: block.def.name + '.html', bytes: block.bytes, sha256: sha256(block.content), rawJavaScriptFragment: true, independentlyParseable: true, includeCountInDashboardMain: count(next, "<?!= includeDashboardFile('" + block.def.name + "') ?>") }; }),
  logicRewritten: false, executionOrderChanged: false, productionDeployment: false
};
fs.mkdirSync(path.dirname(reportPath), { recursive: true });
fs.writeFileSync(reportPath, JSON.stringify(report, null, 2) + '\n', 'utf8');
console.log(JSON.stringify({ dashboardMainBytesBefore: report.sourceBytesBefore, dashboardMainBytesAfter: report.sourceBytesAfter, bytesRemovedFromDashboardMain: report.bytesRemovedFromDashboardMain, moduleCount: report.modules.length, largestModuleBytes: Math.max.apply(null, report.modules.map(function(module) { return module.bytes; })), compositionByteForByteReconstructionVerified: report.compositionByteForByteReconstructionVerified }, null, 2));
